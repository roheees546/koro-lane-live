    ...
    <div className="min-h-screen bg-[#F6F3EE] text-[#111111] font-sans pb-12 selection:bg-[#FF3B30] selection:text-white overflow-x-hidden">
      ...
      <header className="px-6 py-5 flex justify-between items-center relative z-20 border-b border-gray-200 bg-[#F6F3EE]/95 backdrop-blur-md">
        ...
        <div className="flex items-center gap-4">
          <button onClick={() => setShowNotifications(true)} className="text-gray-600 hover:text-[#111111] transition relative">
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"></path></svg>
            <span className="absolute top-0 right-0 w-2.5 h-2.5 bg-[#FF3B30] rounded-full border-2 border-[#F6F3EE]"></span>
          </button>
          <button onClick={() => setShowSettingsModal(true)} className="text-gray-600 hover:text-[#111111] transition">
      ...
      <main className="relative mt-4">
            ...
            <div onClick={() => setShowProfileModal(true)} className="relative p-5 bg-[#FFFFFF] border border-gray-200 rounded-[24px] cursor-pointer hover:border-[#FF3B30]/50 transition duration-300 group overflow-hidden shadow-sm">
               <div className="flex items-center gap-4 relative z-10">
                  ...
                  <div className="flex-1">
                     <h2 className="text-xl font-black text-[#111111] capitalize tracking-tight">{fullName}</h2>
                  ...
                  <div className="flex items-center gap-1 text-[9px] font-black text-gray-400 uppercase tracking-widest group-hover:text-[#FF3B30] transition">
        ...
        {activeView === 'tracking' && (
          <div className="px-6 space-y-4 animate-in slide-in-from-right duration-300">
            ...
            <h2 className="text-sm font-black text-[#111111] uppercase tracking-widest mb-4 flex items-center gap-2"><span className="text-[#FF3B30]">📦</span> Your Secured Drops</h2>
            {orders.map((order) => (
              <div key={order.id} className="bg-[#FFFFFF] border border-gray-200 rounded-[24px] p-5 space-y-4 relative overflow-hidden shadow-sm">
                <div className="flex justify-between items-start">
                  <div><span className="text-[8px] text-[#FF3B30] font-black uppercase tracking-widest bg-[#FCECEC] px-2 py-0.5 rounded border border-red-100">Order Secured</span><h3 className="font-black text-base uppercase text-[#111111] mt-1.5">{order.product_name}</h3><p className="text-[10px] text-gray-500 font-mono mt-0.5">ID: #{order.id.substring(0,8).toUpperCase()}</p></div>
                  <p className="text-lg font-black text-[#111111]">₹{order.price.toLocaleString('en-IN')}</p>
                </div>
                <div className="bg-[#F6F3EE] border border-gray-200 rounded-xl p-4 space-y-3">
                  <div className="flex justify-between text-[9px] font-black uppercase tracking-widest">
                    <span className={order.payment_status === "Pending WhatsApp Confirmation" ? "text-yellow-600" : "text-gray-400"}>1. WA Check</span>
                    <span className={order.status === "packed" ? "text-orange-600" : order.status === "dispatched" || order.status === "delivered" ? "text-orange-600/50" : "text-gray-400"}>2. Packed</span>
                    <span className={order.status === "dispatched" || order.status === "delivered" ? "text-[#FF3B30]" : "text-gray-400"}>3. Dispatched</span>
                  </div>
                  <div className="w-full h-1.5 bg-gray-200 rounded-full relative overflow-hidden">
                    <div className={`h-full absolute left-0 top-0 transition-all duration-500 ${order.status === 'delivered' || order.status === 'dispatched' ? 'w-full bg-[#FF3B30]' : order.status === 'packed' ? 'w-2/3 bg-orange-500' : 'w-1/3 bg-yellow-500'}`} />
                  </div>
                </div>
              </div>
            ))}
            {orders.length === 0 && <div className="text-center py-12 border border-gray-200 border-dashed rounded-[24px] bg-white shadow-sm"><p className="text-gray-500 text-[10px] uppercase tracking-widest font-black">No active orders found.</p></div>}
          </div>
        )}
        ...
        {activeView === 'wishlist' && (
          <div className="px-6 space-y-4 animate-in slide-in-from-right duration-300">
            <button onClick={() => setActiveView('dashboard')} className="flex items-center gap-2 text-gray-500 hover:text-[#111111] text-[10px] font-black uppercase tracking-widest mb-6 transition"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 19l-7-7 7-7"></path></svg>Back to Dashboard</button>
            <h2 className="text-sm font-black text-[#111111] uppercase tracking-widest mb-4 flex items-center gap-2"><span>♡</span> My Wishlist ({wishlistItems.length})</h2>
                ...
                {wishlistItems.map((item, idx) => {
                  ...
                  return (
                    <div 
                      ...
                      className="bg-[#FFFFFF] border border-gray-200 rounded-[20px] p-3 flex flex-col justify-between cursor-pointer hover:border-[#FF3B30]/50 transition group shadow-sm"
                    >
                      {product?.image_url ? (
                        <img src={product.image_url} className="w-full aspect-square object-cover rounded-[14px] mb-2" />
                      ) : (
                        <div className="w-full aspect-square bg-[#F6F3EE] rounded-[14px] flex items-center justify-center text-xl mb-2">👕</div>
                      )}
                      <div>
                        <h4 className="text-xs font-bold text-[#111111] truncate group-hover:text-[#FF3B30] transition">{product?.title || "Exclusive Drop"}</h4>
                        <p className="text-[10px] text-[#FF3B30] font-black mt-0.5">₹{product?.price || "---"}</p>
                      </div>
                    </div>
                  );
                })}
              ...
              followingList.map((follow, idx) => {
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      {seller?.avatar_url ? (
                         <img src={seller.avatar_url} className="w-10 h-10 rounded-full object-cover border border-gray-200" />
                      ) : (
                         <div className="w-10 h-10 bg-[#F6F3EE] rounded-full border border-gray-200 flex items-center justify-center font-black text-xs text-[#FF3B30]">{seller?.store_name?.[0] || seller?.full_name?.[0] || "S"}</div>
                      )}
                      <div>
                        <h4 className="text-xs font-bold text-[#111111] group-hover:text-[#FF3B30] transition">{seller?.store_name || seller?.full_name || "Unknown Seller"}</h4>
                    ...
                    <svg className="w-4 h-4 text-gray-400 group-hover:text-[#111111] transition" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5l7 7-7 7"></path></svg>
                  </div>
                )
              })
            ) : (
