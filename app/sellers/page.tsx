...
export default function SellersPage() {
  ...
  return (
    <div className="min-h-screen bg-black text-white w-full pb-24">
      ...
      {/* 🚀 SELLERS LIST (Updated to Grid Layout) */}
      <div className="p-4">
        {loading ? (
          ...
          <div className="grid grid-cols-2 gap-3">
            {sellers.map((seller) => (
              <Link href={`/store/${seller.id}`} key={seller.id} className="bg-[#121214] border border-gray-800 rounded-xl p-4 flex flex-col items-center text-center relative overflow-hidden hover:border-[#00e599]/50 transition group block">
                <div className="absolute left-0 right-0 top-0 h-16 bg-gradient-to-b from-[#003320]/20 to-transparent pointer-events-none"></div>
                
                <div className="w-14 h-14 bg-black border border-gray-700 rounded-full flex items-center justify-center shrink-0 overflow-hidden shadow-inner mb-3 z-10">
                ...
                <div className="flex flex-col items-center w-full z-10">
                  <span className="bg-[#003320] text-[#00e599] text-[6px] font-bold px-1.5 py-0.5 rounded uppercase tracking-widest mb-1.5 inline-block">Verified</span>
                  <h4 className="text-xs font-black text-white flex items-center justify-center gap-1 w-full truncate group-hover:text-[#00e599] transition">
                    {seller.store_name || "VERIFIED DEALER"} <svg className="w-2.5 h-2.5 text-[#00e599] shrink-0" fill="currentColor" viewBox="0 0 24 24"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"/></svg>
                  </h4>
                  <p className="text-[8px] text-gray-500 mt-1 line-clamp-1">{seller.address || 'Dehradun, Uttarakhand'}</p>
                  
                  <div className="mt-3 w-full border-t border-gray-800 pt-3 flex justify-between items-center">
                    ...
                    <span className="text-[8px] text-[#00e599] font-black uppercase tracking-widest">Visit →</span>
                  </div>
                </div>
              </Link>
            ))}
