...
export default function DealerDashboard() {
  ...
  const fetchDashboardData = async () => {
    ...
    if (!profile || intendedRole === 'seller') {
      const { data: savedProfile } = await supabase.from("profiles").upsert({
        id: session.user.id,
        email: sessionEmail,
        role: "dealer",
  ...
  const handleAddItem = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from("products").insert([{
      ...
      size: itemCategory === 'Top' ? 'Free Size' : 'Standard', // Safe fallback for DB
    ...
    if (!error) {
      ...
      setItemName(""); setItemPrice(""); setItemDesc(""); setImageFiles([]); 
            ...
            <div className="p-6 overflow-y-auto hide-scrollbar flex-1">
              <form id="add-product-form" onSubmit={handleAddItem} className="space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  {/* CATEGORY (Full width, Top & Bottom only) */}
                  <div>
            ...
            <div className="p-6 overflow-y-auto hide-scrollbar space-y-8">
              ...
              <div className="bg-[#1a1a1d] border border-gray-800 rounded-2xl p-6">
                <div className="flex items-center gap-2 mb-6 border-b border-gray-800/50 pb-3">
                  <h3 className="text-white font-black text-sm tracking-widest uppercase">Top Wear Guide</h3>
                </div>
              ...
              <div className="bg-[#1a1a1d] border border-gray-800 rounded-2xl p-6">
                <div className="flex items-center gap-2 mb-6 border-b border-gray-800/50 pb-3">
                  <h3 className="text-white font-black text-sm tracking-widest uppercase">Bottom Wear Guide</h3>
                </div>
                <div className="space-y-4 text-xs text-gray-300">
