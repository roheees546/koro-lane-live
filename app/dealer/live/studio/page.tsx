...
export default function PreLiveStudio() {
  ...
  // Fetch dealer's real inventory to show in the Live Studio!
  useEffect(() => {
  ...
  const handleGoLive = () => {
    ...
    setIsLive(true);
    // Real logic later: Trigger WebRTC/YouTube stream here
  };
  ...
  return (
    <div className="relative h-[100dvh] w-full max-w-[450px] mx-auto bg-black text-white overflow-hidden selection:bg-[#00e599] selection:text-black">
      ...
      {/* 🎥 FAKE CAMERA BACKGROUND (Full Screen) */}
      <div className={`absolute inset-0 flex flex-col items-center justify-center transition duration-700 ${isLive ? 'bg-zinc-900' : 'bg-zinc-950'}`}>
        {!isLive ? (
          <>
            <svg className="w-16 h-16 text-zinc-800 mb-4 animate-pulse" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z"></path></svg>
            <p className="text-zinc-700 font-black uppercase tracking-widest text-xs">Camera Preview</p>
          </>
        ) : (
      ...
      <div className="absolute top-0 w-full p-4 pt-safe-top flex justify-between items-start bg-gradient-to-b from-black/90 to-transparent z-20 h-32">
        <div className="flex flex-col gap-3">
          <button onClick={() => isLive ? alert("End stream first!") : router.back()} className="w-10 h-10 bg-black/40 backdrop-blur-md rounded-full flex items-center justify-center border border-white/10 text-white hover:bg-black/60 transition">
        ...
        <div className="flex flex-col gap-3">
          <button className="w-10 h-10 bg-black/40 backdrop-blur-md rounded-full flex items-center justify-center border border-white/10 text-white hover:bg-black/60 transition">
      ...
      {!isLive && (
        <div className="absolute bottom-0 w-full p-5 bg-gradient-to-t from-black via-black/95 to-transparent z-20 pt-20">
          <div className="space-y-5">
            ...
            <div>
              ...
              <input 
                ...
                maxLength={40}
                className="w-full bg-[#121214]/80 backdrop-blur-md border border-gray-800 rounded-2xl px-4 py-4 text-sm focus:outline-none focus:border-[#00e599] transition mt-1.5 shadow-lg"
              />
            </div>

            {/* Product Linker Button */}
            <button 
      ...
      {isLive && (
        <div className="absolute bottom-0 w-full p-4 bg-gradient-to-t from-black via-black/80 to-transparent z-20 pt-20 flex flex-col gap-4">
          ...
          {/* Active Pinned Product (Visible to buyers) */}
          {pinnedProduct ? (
          ...
          <div className="flex gap-3">
            ...
            <button onClick={() => setIsLive(false)} className="w-14 bg-red-600/20 border border-red-600/50 text-red-500 rounded-2xl flex items-center justify-center hover:bg-red-600/30 transition">
      ...
      {/* 🛍️ PRODUCT SELECTOR MODAL (Slides up) */}
      {showProductModal && (
              ...
              {inventory.length === 0 ? (
                ...
                inventory.map(item => (
                  <div key={item.id} className="flex items-center gap-3 p-3 bg-[#0a0a0c] border border-gray-800 rounded-2xl">
                    ...
                    {/* Checkbox for Setup vs 'Pin' button for Live */}
                    {!isLive ? (
